...
![img](%E6%95%B0%E6%8D%AE%E5%BA%93%E5%9F%BA%E7%A1%80.assets/1602314271999-8d553c6e-22ea-4190-973e-16672627584c.png)
...
SQL语句在查询之前会使用查询优化器对查询进行优化。就是优化客户端请求的 query（SQL 语句） ，根据客户端请求的 query 语句，和数据库中的一些统计信息，在一系列算法的基础上进行分析，得出一个最优的策略，告诉后面的程序如何取得这个 query 语句的结果
...
![img](%E6%95%B0%E6%8D%AE%E5%BA%93%E5%9F%BA%E7%A1%80.assets/1622198425194-7883b8a6-e904-48c4-9b12-f85eeeac237e.png)

当 MySQL 启动（MySQL 服务器就是一个进程），等待客户端连接，每一个 客户端连接请求，服务器都会新建一个**线程**处理（如果是线程池的话，则是分配一个空的线程），每个线程独立，拥有各自的内存处理空间。

连接到服务器，服务器需要对其进行**验证**，也就是用户名、IP、密码验证， 一旦连接成功，还要验证是否具有执行某个特定查询的权限（例如，是否允许客 户端对某个数据库某个表的某个操作）
...
<img src="数据库基础.assets/image-20210616210540633.png" alt="image-20210616210540633" style="zoom:80%;" />
...
data.txt文件内容为（两个值用一个tab间隔，按建表时的变量声明顺序排列，使用\N代表NULL值）：
...
通过过滤选择出不具有特定值的行时，结果中不会包含具有 NULL 的行，因为 NULL 有特殊的含义，无法进行匹配。
...
在MySQL中，0或NULL代表false，其他值代表true。
...
出现在 SELECT 后的子查询仅支持**标量子查询（一行一列）**：
...
- `SubString(str, pos)`：返回子串的字符，**索引从1开始**。
...
`custnew`表中的数据结构与customers表相同，在插入时注意不能使用重复的`cust_id`，可以直接省略`cust_id`。
...
**truncate 和 delete 的区别：**对采用自增主码的表，truncate删除后，再插入数据时，标识从1开始，而 delete 后标识从断点开始；truncate 不可以回滚，delete 可以回滚。
...
MYSQL中两个最常用的数据库引擎为`InnoDB`和`MyISAM`，后者支持全文本搜索，而前者不支持。
...
在 MySQL命令行的默认设置下，事务都是自动提交( auto commit）的，即执行SQL语句后就会马上执行COMMIT操作。因此要显式地开启一个事务需使用命令BEGIN、START TRANSACTION，或者执行命令SET AUTOCOMMIT=O，禁用当前会话的自动提交。

事务控制语句：

- `START TRANSACTOPM|BEGIN`：显式地开启一个事务
- `COMMIT|COMMIT WORK`：提交事务
- `ROLLBACK|ROLLBACK WORK`：结束当前事务，撤销所有未提交的修改
- `SAVEPOINT identifier`：在事务中创建一个保存点
- `RELEASE SAVEPOINT identifier`：删除一个保存点
- `ROLLBACK TO SAVEPOINT identifier`：将事务回滚到保存点
- `SET TRANSACTION`：设置事务的隔离级别

在存储过程中，BEGIN与存储过程的BEGIN冲突，只能使用`START TRANSACTION`。

COMMIT和 COMMIT WORK语句基本是一致的，都是用来提交事务。不同之处在于COMMIT WORK用来控制事务结束后的行为是CHAIN还是RELEASE的。如果是CHAIN方式，那么事务就变成了链事务。

用户可以通过参数completion_type来进行控制，该参数默认为0，表示没有任何操作。在这种设置下COMMIT和 COMMIT WORK是完全等价的。当参数completion_type的值为1时，COMMIT WORK 等同于COMMIT AND CHAIN，表示马上自动开启一个相同隔离级别的事务。

当参数completion_type的值为2时，COMMIT WORK 等同于COMMIT AND RELEASE，表示事务完成后断开与服务器的连接。
