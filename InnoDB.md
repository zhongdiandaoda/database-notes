...
数据库中B+树的索引分为聚簇索引和辅助索引，二者内部都是高度平衡的，叶子节点存放着所有的数据。

聚簇索引和辅助索引的区别是：叶子节点存放的是否是一整行的信息。

`InnoDB`存储引擎表是索引组织表，表中数据按照主键顺序存放。聚簇索引就是按照每张表的主键构建一棵二叉树，同时，叶子节点存放的是整张表的行记录数据，聚簇索引的叶子节点也被称为数据叶，每个数据叶都通过一个双向链表进行链接。
